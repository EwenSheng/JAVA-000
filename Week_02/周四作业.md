...
5. 面对吞吐量大的场景下建议使用 Parallel GC > CMS GC ≈ G1 GC > Serial GC
6. 面对需要响应速度的场景下G1 > CMS GC  ≈ Parallel GC > Serial GC
~~~
   G1 / CMS 是并发GC,面对老年代存活对象处理时长问题使用了标记+并发清除，在回收时减少了STW时间,因此对应用线程影响较小，使得平均响应速度更快; (性能上较为平滑)
        CMS 且因为是并发执行，所以Full GC效率没有并行的高；因为不压缩所以会导致老年代的内存碎片问题，在某些情况下GC会造成不可预测的暂停时间，特别是堆内存较大的情况（理解上是因为使用了空闲列表[free-lists]的原因）,并且CMS会在并发阶段与应用线程抢占CPU时间;
   Parallel GC 并行执行,由于年轻代/老年代的垃圾回收行为都会触发STW（GC期间为了减少SWT时间所有CPU并行执行）,但对要求响应速度的应用线程会有一定影响（GC时会性能有影响）
   Serial GC 单核单线程,串行性能差
~~~